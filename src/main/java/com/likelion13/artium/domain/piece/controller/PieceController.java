...
public class PieceController {
  ...
  public ResponseEntity<BaseResponse<String>> createPiece(
      @AuthenticationPrincipal CustomUserDetails userDetails,
      @Parameter(description = "작품 등록 내용") @RequestPart("data") @Valid CreatePieceRequest createPieceRequest,
      @Parameter(description = "작품 메인 이미지")@RequestPart("image") MultipartFile image,
      @Parameter(description = "작품 디테일 컷 모음") @RequestPart("detailImages") List<MultipartFile> pieceDetails) {

    Long userId = userDetails.getUser().getId();

    //PieceResponse pieceResponse = pieceService.createPiece(userId, createPieceRequest, image, pieceDetails);

    String testResponse = pieceService.createPiece(userId, createPieceRequest, image, pieceDetails);
    return ResponseEntity.ok(BaseResponse.success("작품 등록에 성공했습니다.", testResponse));
  }

  @Operation(summary = "특정 작품 정보 조회하기 API", description = "특정 작품 정보 수정 및 작품 상세 정보에서 작품 정보를 조회하기 위한 API")
  @GetMapping("/{piece-id}")
  ...
  public ResponseEntity<BaseResponse<PieceResponse>> updatePiece(
      @Parameter(description = "특정 작품 ID") @PathVariable(value = "piece-id") Long pieceId,
      @AuthenticationPrincipal CustomUserDetails userDetails,
      @Parameter(description = "작품 수정 내용") @RequestPart("data") @Valid UpdatePieceRequest updatePieceRequest,
